...
export async function WebinarsPage() {
  try {
    ...
    const webinarList = await axiosInstance
      .get<{
        results: IUpcomingWebinarListItem[];
      }>("academy/webinar_list/")
      .then((res) => res?.data.results);
    const webinarAfterwards = await axiosInstance
      .get<{
      ...
      <Fragment>
        ...
        >
          ...
          >
            ...
            технологий в формате онлайн-вебинара.
            В вебинаре (онлайн лекции) всегда
            участвуют две стороны:
            докладчик-ведущий и слушатели. Обычно
            участники могут видеть ведущего, а он
            их - нет. Наши лекции по исламу
            сегодня одновременно посещают от 100
            до 1000 человек.
          </Typography>
        </Banner>
        <SectionHeader color="primary">
